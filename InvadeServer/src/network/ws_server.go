...
func (handler *WSHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		llog.Debug("upgrade error: %v", err)
		return
	}
...
func (server *WSServer) Start() {
	...
	if server.MaxConnNum <= 0 {
		...
		llog.Release("invalid MaxConnNum, reset to %v", server.MaxConnNum)
	}
	if server.PendingWriteNum <= 0 {
		server.PendingWriteNum = 100
		llog.Release("invalid PendingWriteNum, reset to %v", server.PendingWriteNum)
	}
	if server.MaxMsgLen <= 0 {
		server.MaxMsgLen = 4096
		llog.Release("invalid MaxMsgLen, reset to %v", server.MaxMsgLen)
	}
	if server.HTTPTimeout <= 0 {
		server.HTTPTimeout = 10 * time.Second
		llog.Release("invalid HTTPTimeout, reset to %v", server.HTTPTimeout)
	}

...
google.pacman ||
    function () {
        ...
        E.prototype.initActorData = function () {//创建角色
            ...
            //this.posDelta = [0, 0];
            //this.tilePos = [b.y * 8, b.x * 8];//actor所在格子左上角
        ...
        E.prototype.changeActorMode = function (newMode) {
            //    var oldMode = this.mode;
            this.mode = newMode;
            //    if (this.id == g.playerCount + 3 && (newMode == 16 || oldMode == 16)) g.updateCruiseElroySpeed();
            //    switch (oldMode) {
            //        case 32:
            //            g.ghostExitingPenNow = false;//e;
            //            break;
            //        case 8:
        ...
        //设置怪物的目标玩家id(针对多个玩家时)
        E.prototype.setGhostTargetPlayerId = function () {
        ...
        //                else {
        //                    do f = directionEnum[Math.floor(g.rand() * 4)];
        ...
        //    }
        //};
        //E.prototype.p = function (b) {//updateTilePos
        //    g.tilesChanged = true;
        ...
        //        b = directionIncrement[this.nextDir];
        ...
        //    }
        //    this.mode == 8 && this.pos[0] == s[0] && this.pos[1] == s[1] && this.changeActorMode(64);
        ...
        //            case g.playerCount + 1:
        //                this.targetPos = [b.tilePos[0], b.tilePos[1]];
        //                var c = directionIncrement[b.dir];
        ...
        //        case 0:
        //            var tempSpeed = this.id == g.playerCount && (this.mode == 2 || this.mode == 1) ? g.cruiseElroySpeed : this.fullSpeed;
        //            break;
        //        case 1:
        ...
        E.prototype.e = function () {
            if (this.dir) {
                if (this.speedIntervals[g.intervalTime]) {
                    var b = directionIncrement[this.dir];
        ...
        E.prototype.updateRenderPos = function () {
        ...
        g.getPlayfieldX = function (x) {
            return x + -32
        };
        g.getPlayfieldY = function (y) {
            return y + 0
        };
        ...
        g.getSpeedIntervals = function (speed) {
            if (!g.speedIntervals[speed]) {
                var c = 0,
                    d = 0;
                g.speedIntervals[speed] = [];
                for (var i = 0; i < D; i++) {
                    c += speed;
                    if (Math.floor(c) > d) {
                        g.speedIntervals[speed].push(a);
                        d = Math.floor(c)
                    }
                    else
                        g.speedIntervals[speed].push(e)
                }
            }
            return g.speedIntervals[speed]
        };
        g.finishLevel = function () {
            g.changeGameplayMode(9)
        };
        g.changeGameplayMode = function (_mode) {
            g.gameplayMode = _mode;
            //if (_mode != 13) {
        ...
        g.createCanvasElement = function () {//创建画布canvas
        ...
        g.everythingIsReady = function () {//声音和图片资源加载完成之后调用这个
            if (!g.ready) {
                g.ready = true;
        ...
        g.createActors = function () {
            g.actors = [];
            for (var i = 0; i < g.playerCount + 4; i++) {
                g.actors[i] = new E(i);
                if (i < g.playerCount) {
                    g.actors[i].ghost = false;
                    g.actors[i].mode = 1
                }
                else
                    g.actors[i].ghost = true;
            }
        };
        ...
        g.resetPlayfield = function () {
            ...
            g.determinePlayfieldDimensions();//确定地图宽高
            g.preparePlayfield();//
            g.preparePaths();
        ...
        g.createActorElements = function () {
            for (var idx in g.actors)
                g.actors[idx].createElement()
        };
        //初始化地图
        g.preparePlayfield = function () {
            g.playfield = [];
            for (var h = 0; h <= g.playfieldHeight + 1; h++) {
                g.playfield[h * 8] = [];
                for (var w = -2; w <= g.playfieldWidth + 1; w++) {
                    g.playfield[h * 8][w * 8] = {
                        path: 0,
                        dot: 0,
                        intersection: 0
        ...
        g.determinePlayfieldDimensions = function () {
            g.playfieldWidth = 0;//可移动到的宽度
            g.playfieldHeight = 0;//可移动到的高度
            for (var b in allPaths) {
                var c = allPaths[b];
                if (c.w) {
                    c = c.x + c.w - 1;
        ...
        g.preparePaths = function () {//解析路径数据
            //给路径上加点,类型
            for (var idx in allPaths) {
                ...
                if (path.w) {
                    y = path.y * 8;
                    for (var x = path.x * 8; x <= (path.x + path.w - 1) * 8; x += 8) {
                        g.playfield[y][x].path = true;//a;
                        if (g.playfield[y][x].dot == 0) {
                            g.playfield[y][x].dot = 1;
                            g.dotsRemaining++
                        }
                        g.playfield[y][x].type = (!d || x != path.x * 8 && x != (path.x + path.w - 1) * 8 ? d : 0)
                    }
                    //头尾是交叉路口
                    g.playfield[y][path.x * 8].intersection = true;
                    g.playfield[y][(path.x + path.w - 1) * 8].intersection = true
                }
                else {
                    x = path.x * 8;
                    for (y = path.y * 8; y <= (path.y + path.h - 1) * 8; y += 8) {
                        if (g.playfield[y][x].path)
                            g.playfield[y][x].intersection = true;
                        g.playfield[y][x].path = true;
                        if (g.playfield[y][x].dot == 0) {
                            g.playfield[y][x].dot = 1;
                            g.dotsRemaining++
                        }
                        g.playfield[y][x].type = (!d || y != path.y * 8 && y != (path.y + path.h - 1) * 8 ? d : 0)
                    }
                    g.playfield[path.y * 8][x].intersection = true;
                    g.playfield[(path.y + path.h - 1) * 8][x].intersection = true
                }
            }
            //去除掉没有点的路径上的点
            for (idx in noDotPaths) {
                ...
                if (path.w) {
                    for (x = path.x * 8; x <= (path.x + path.w - 1) * 8; x += 8) {
                        g.playfield[path.y * 8][x].dot = 0;
                        g.dotsRemaining--
                    }
                }
                else {
                    for (y = path.y * 8; y <= (path.y + path.h - 1) * 8; y += 8) {
                        g.playfield[y][path.x * 8].dot = 0;
                        g.dotsRemaining--
                    }
                }
            }
        ...
        g.createDotElements = function () {
            for (var y = 8; y <= g.playfieldHeight * 8; y += 8) {
                for (var x = 8; x <= g.playfieldWidth * 8; x += 8) {
                    if (g.playfield[y][x].dot) {
                        var d = document.createElement("div");
                        d.className = "pcm-d";
                        d.id = g.getDotElementId(y, x);
        ...
        g.checkIfEverythingIsReady = function () {
            if (g.soundReady || g.graphicsReady)//资源还没建在完是显示进度
                g.updateLoadingProgress(0.67);
            if (g.soundReady && g.graphicsReady) {
                g.updateLoadingProgress(1);//进度条设成100%
                g.everythingIsReady()
            }
        };
